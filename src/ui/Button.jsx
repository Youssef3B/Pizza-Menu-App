...
function Button({ children, disabled, to, type, onClick }) {
  ...
  const styles = {
    primary: base + ' px-4 py-3 md:px-4 md:py-3',
    small: base + ' px-4 py-2 md:px-4 md:py-2 text-xs',
    round: base + ' px-2.5 py-1 md:px-3.5 md:py-2 text-sm',
    secondary:
      'inline-block text-sm rounded-full bg-transparent border-2 border-stone-300  font-semibold uppercase tracking-wide text-stone-400 transition-all duration-300 hover:bg-stone-300 focus:outline-none focus:ring focus:ring-stone-300 focus:ring-offset-2 disabled:cursor-not-allowed px-4 py-3 md:px-4 md:py-3 hover:text-stone-800',
  };
  if (to)
    return (
